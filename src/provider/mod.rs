...
use crate::{
    auth::{self, AuthorizationRequestData, Challenge, ChallengeInfo},
    core::models::{ClientInfo, Consent, ConsentId},
};
use crate::{
    auth::{
        ...
        ClientCredentials, Redirect, Store, UpdateChallengeDataRequest,
        UpdateChallengeDataResponse,
    },
...
impl OAuth2Provider {
    ...
    async fn check_client_authentication(
        ...
        if let Ok(Some(c)) = client {
            let result = self.hasher.verify(&cred.client_secret, &c.secret);
    ...
    async fn start_clean_up_worker(&self) -> Result<(), Error> {
        ...
        loop {
            ...
            event!(
                Level::TRACE,
                "Cleaning invalid entries codes, tokens, and challenges"
            );
            self.store.clean_up()?;
        }
    }
...
impl OAuth2Provider {
    fn validate_token_for_challenge(&self, token: BearerToken) -> Result<(), Error> {
        self.token
            .validate_token_contains(token, "tomiko::challenge:rw")
    }

    fn validate_token_for_consent(&self, token: BearerToken) -> Result<(), Error> {
        self.token
            .validate_token_contains(token, "tomiko::consent:rw")
    }

    fn validate_token_for_client_info(&self, token: BearerToken) -> Result<(), Error> {
        self.token
            .validate_token_contains(token, "tomiko::client:ro")
    }
    ...
    pub async fn get_challenge_info(
        ...
        event!(Level::DEBUG, "Retrieving challenge info");
    ...
    pub async fn get_challenge_result(
        ...
        event!(Level::DEBUG, "Getting final challenge outcome");
        let info = self
            .store
            .get_challenge_data(&id)
            .and_then(|info| {
        ...
        if !info.ok {
            event!(Level::WARN, "Challenge was not passed");
        ...
        match info.req {
            ...
            | AuthorizationRequest::ImplicitId(AuthorizationRequestData {
                ref client_id, ..
            }) => {
                event!(
                    Level::DEBUG,
    ...
    #[tracing::instrument(skip(self, token))]
    pub async fn get_consent(&self, token: BearerToken, id: ConsentId) -> Result<Consent, Error> {
    ...
    #[tracing::instrument(skip(self, token))]
    pub async fn put_consent(&self, token: BearerToken, consent: Consent) -> Result<(), Error> {
    ...
    #[tracing::instrument(skip(self, token))]
    pub async fn revoke_consent(&self, token: BearerToken, id: ConsentId) -> Result<(), Error> {
...
impl Config {
    pub fn from_env() -> Self {
        Self {
            ...
            self_base: std::env::var("SELF_HTTP_BASE").expect("Supply SELF_HTTP_BASE"),
            issuer_prefix: std::env::var("ISSUER_PREFIX").expect("Supply ISSUER_PREFIX"),
        }
    }
}
