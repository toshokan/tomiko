...
use diesel::prelude::*;
use diesel::r2d2::{Pool, PooledConnection, ConnectionManager, Builder as PoolBuilder};
...
impl DbStore {
    ...
    pub fn migrate(&self) {
	embedded_migrations::run_with_output(&self.conn(), &mut std::io::stderr())
	    .unwrap();
...
impl Store for  DbStore {
    ...
    fn put_client(&self, id: ClientId, name: String, secret: HashedClientSecret) -> Result<Client, Error> {
	use schema::clients::dsl::clients;
    ...
    fn take_authcode_data(&self, client_id: &ClientId, code: &HashedAuthCode) -> Result<AuthCodeData, Error> {
	...
	block_in_place(|| {
	    diesel::delete(
		codes.filter(codes_dsl::invalid_after.lt(time))
	    ).execute(&self.conn())
	})?;

	block_in_place(|| {
	    diesel::delete(
		refresh_tokens.filter(tokens_dsl::invalid_after.lt(time))
	    ).execute(&self.conn())
	})?;

	block_in_place(|| {
	    diesel::delete(
		challenges.filter(challenges_dsl::invalid_after.lt(time))
	    ).execute(&self.conn())
	})?;
	...
	let allowed_scopes = block_in_place(|| {
	    client_scopes.filter(dsl::client_id.eq(&client_id.0))
...
impl DbStore {
	...
	Ok(consents.drain(..).map(|c| Consent {
