...
pub fn consent_endpoint(
    ...
    let get = warp::path!("consent")
        ...
        .and_then(|id, provider: Arc<OAuth2Provider>, token| async move {
            reply::json_encode(provider.get_consent(token, id).await)
        });
    ...
    let all = warp::path!("consent" / String)
        ...
        .and(encoding::bearer())
        .and_then(
            |subject: String, provider: Arc<OAuth2Provider>, token| async move {
                reply::json_encode(provider.get_all_consents(token, subject).await)
            },
        );
    ...
    let set = warp::path!("consent")
        ...
        .and_then(|consent, provider: Arc<OAuth2Provider>, token| async move {
            reply::json_encode(provider.put_consent(token, consent).await)
        });
    ...
    let revoke = warp::path!("revoke")
        ...
        .and(encoding::bearer())
        .and_then(|id, provider: Arc<OAuth2Provider>, token| async move {
            reply::json_encode(provider.revoke_consent(token, id).await)
        });

    warp::path("v1").and(get.or(all).or(set).or(revoke))
}
