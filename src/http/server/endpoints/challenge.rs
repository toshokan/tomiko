...
use crate::{core::types::ChallengeId, provider::OAuth2Provider};

pub fn challenge_endpoint(
    ...
    let update = warp::path!("data" / ChallengeId)
        ...
        .and(warp::body::json())
        .and(encoding::bearer())
        .and_then(|id, provider: Arc<OAuth2Provider>, req, token| async move {
            provider
    ...
    warp::path("v1").and(info.or(update).or(cont))
}
