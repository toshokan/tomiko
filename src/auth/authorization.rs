use crate::auth::{oidc, pkce, AccessTokenResponse};

use crate::core::types::AuthCode;
use crate::{
    auth::error::{ErrorKind, ErrorResponse},
    core::types::{ClientId, RedirectUri, Scope},
};

use super::{ImplicitRequestExt, MaybeRedirect, WithState};

pub type AuthorizationErrorResponse = WithState<ErrorResponse<AuthorizationErrorKind>>;
pub type AuthorizationError =
    MaybeRedirect<AuthorizationErrorResponse, AuthorizationRedirectErrorKind>;
...
pub enum AuthorizationRequest {
    ...
    Implicit(
        AuthorizationRequestData<
            ImplicitRequestExt<Option<oidc::ImplicitGrantAuthorizationRequest>>,
        >,
    ),
    #[serde(rename = "id_token token")]
    ImplicitId(
        AuthorizationRequestData<ImplicitRequestExt<oidc::ImplicitGrantAuthorizationRequest>>,
    ),
}
...
pub struct AuthorizationCodeRequestExt {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(
        deserialize_with = "oidc::AuthorizationCodeGrantAuthorizationRequest::deserialize_skip_default"
    )]
